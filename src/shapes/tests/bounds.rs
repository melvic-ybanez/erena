...
fn test_bounded_cylinder_bounds() {
    let bbox = CylLike::cylinder().min(-5.0).max(3.0).to_shape().bounds();
...
fn test_unbounded_cone_bounds() {
    ...
    assert_eq!(
        bbox.min,
        points::new(-Real::INFINITY, -Real::INFINITY, -Real::INFINITY)
    );
    assert_eq!(
        bbox.max,
        points::new(Real::INFINITY, Real::INFINITY, Real::INFINITY)
...
fn test_bounded_cone_bounds() {
    let bbox = CylLike::cone().min(-5.0).max(3.0).to_shape().bounds();
...
fn text_box_containing_box() {
    ...
    let data = [
        (
            points::new(5.0, -2.0, 0.0),
            points::new(11.0, 4.0, 7.0),
            true,
        ),
        (
            points::new(6.0, -1.0, 1.0),
            points::new(10.0, 3.0, 6.0),
            true,
        ),
        (
            points::new(4.0, -3.0, -1.0),
            points::new(10.0, 3.0, 6.0),
            false,
        ),
        (
            points::new(6.0, -1.0, 1.0),
            points::new(12.0, 5.0, 8.0),
            false,
        ),
    ];
...
fn test_bounding_box_transformation() {
    ...
    assert_eq!(
        bbox.min.round_items(),
        points::new(-1.41421, -1.70711, -1.70711)
    );
    assert_eq!(
        bbox.max.round_items(),
        points::new(1.41421, 1.70711, 1.70711)
    );
}
