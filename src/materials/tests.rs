...
fn test_lighting_with_pattern() {
    let mut mat = Material::default().pattern_ref(&Pattern::stripe(Color::white(), Color::black()));
    mat.ambient = 1.0;
    mat.diffuse = 0.0;
    mat.specular = 0.0;
    ...
    let normal_vec = vectors::new(0.0, 0.0, -1.0);
    let light = PointLight::new(points::new(0.0, 0.0, -10.0), Color::white());
    let c1 = mat.pl_lighting(
        &Shape::sphere(),
        light,
        points::new(0.9, 0.0, 0.0),
        eye_vec,
        normal_vec,
        1.0,
    );
    let c2 = mat.pl_lighting(
        &Shape::sphere(),
        light,
        points::new(1.1, 0.0, 0.0),
        eye_vec,
        normal_vec,
        1.0,
    );
...
fn test_lighting_uses_intensity() {
    ...
    for (intensity, res) in data {
        let result = shape
            .material
            .pl_lighting(&shape, light, point, eye_vec, normal_vec, intensity);
...
fn test_lighting_samples_area_light() {
    ...
    let data = [
        (
            ...
            colors::new(0.99650, 0.99650, 0.99650),
        ),
        (
            points::new(0.0, 0.7071, -0.7071),
            colors::new(0.62318, 0.62318, 0.62318),
        ),
    ];
    for (point, result) in data {
        ...
        let lighting =
            shape
                .material
                .lighting(&shape, light.clone(), point, eye_vec, normal_vec, 1.0);
