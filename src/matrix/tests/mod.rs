use crate::matrix::{no_inverse, Matrix};
...
#[test]
fn test_2x2_matrix() {
    let m = Matrix::new22(&[-3.0, 5.0, 1.0, -2.0]);
    assert_eq!(m[(0, 0)], -3.0);
    assert_eq!(m[(0, 1)], 5.0);
    assert_eq!(m[(1, 0)], 1.0);
    assert_eq!(m[(1, 1)], -2.0);
...
#[test]
fn test_3x3_matrix() {
    let m = Matrix::new33(&[-3.0, 5.0, 0.0, 1.0, -2.0, -7.0, 0.0, 1.0, 1.0]);
    assert_eq!(m[(0, 0)], -3.0);
    assert_eq!(m[(1, 1)], -2.0);
    assert_eq!(m[(2, 2)], 1.0);
}
...
fn test_2x2_determinant() {
    let matrix = Matrix::new_nxn(2, &[1.0, 5.0, -3.0, 2.0]);
    assert_eq!(matrix.determinant(), 17.0);
}

#[test]
fn test_submatrix() {
    let matrix = Matrix::new33(&[1.0, 5.0, 0.0, -3.0, 2.0, 7.0, 0.0, 6.0, -3.0]);
    assert_eq!(
        matrix.submatrix(0, 2),
        Matrix::new22(&[-3.0, 2.0, 0.0, 6.0,])
    );
    ...
    assert_eq!(
        matrix1.submatrix(2, 1),
        Matrix::new33(&[-6.0, 1.0, 6.0, -8.0, 8.0, 6.0, -7.0, -1.0, 1.0,])
    );
}

#[test]
fn test_minor_for_3x3() {
    let matrix = Matrix::new33(&[3.0, 5.0, 0.0, 2.0, -1.0, -7.0, 6.0, -1.0, 5.0]);
...
fn test_determinant_for_3x3() {
    let matrix = Matrix::new33(&[1.0, 2.0, 6.0, -5.0, 8.0, -4.0, 2.0, 6.0, 4.0]);
...
fn test_not_invertible() {
    let matrix = Matrix::new44i(&[-4, 2, -2, -3, 9, 6, 2, 6, 0, -5, 1, -5, 0, 0, 0, 0]);
...
#[test]
fn test_inverse() {
    let m = Matrix::new44i(&[-5, 2, 6, -8, 1, -5, 1, 8, 7, 7, -6, -7, 1, -3, 7, 4]);
    match m.inverse() {
        Some(mi) => {
    ...
    let m = Matrix::new44i(&[8, -5, 9, 2, 7, 5, 6, 1, -6, 0, 9, 6, -3, 0, -9, -4]);
    match m.inverse() {
        Some(mi) => assert_eq!(
            mi.round_items(5),
            Matrix::new44(&[
    ...
    let m = Matrix::new44i(&[9, 3, 0, 9, -5, -2, -6, -3, -4, 9, 6, 4, -7, 6, 6, 2]);
    match m.inverse() {
        Some(mi) => assert_eq!(
            mi.round_items(5),
            Matrix::new44(&[
...
fn test_multiply_product_by_inverse() {
    let m = Matrix::new44i(&[3, -9, 7, 3, 3, -8, 2, -9, -4, 4, 4, 1, -6, 5, -1, 1]);
    let m1 = Matrix::new44i(&[8, 2, 2, 2, 3, -1, 7, 0, 7, 0, 5, 4, 6, -2, 0, 5]);
    let product = &m * &m1;
    match m1.inverse() {
